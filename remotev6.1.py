...
class RemoteController:
    def __init__(self):
        ...
        pygame.display.set_caption("ESP32 Remote Controller with DC Motor")
        ...
        self.key_states = {
            pygame.K_w: False,  # Motor Forward
            pygame.K_s: False,  # Motor Backward
            pygame.K_a: False,  # Servo Left
            pygame.K_d: False,  # Servo Right
            pygame.K_UP: False,  # Slider increase
            pygame.K_DOWN: False  # Slider decrease
        }
        
        # Motor state tracking
        self.motor_state = "STOP"  # STOP, FORWARD, BACKWARD
        self.last_motor_state = "STOP"
        
        # Servo state tracking
        self.servo_state = "CENTER"  # CENTER, LEFT, RIGHT
        self.last_servo_state = "CENTER"
    ...
    def draw_interface(self):
        ...
        title = self.title_font.render("ESP32 Remote Controller with DC Motor", True, self.primary_color)
        ...
        # Draw motor control indicator (top right)
        motor_rect = pygame.Rect(self.width - 220, 80, 200, 80)
        pygame.draw.rect(self.screen, self.card_color, motor_rect, 0, 10)
        
        # Draw motor status text
        motor_text = self.header_font.render("DC Motor", True, self.primary_color)
        self.screen.blit(motor_text, (motor_rect.centerx - motor_text.get_width()//2, 
                                     motor_rect.top + 10))
        
        # Draw motor state
        if self.motor_state == "FORWARD":
            state_text = self.value_font.render("FORWARD", True, self.success_color)
        elif self.motor_state == "BACKWARD":
            state_text = self.value_font.render("BACKWARD", True, self.warning_color)
        else:
            state_text = self.value_font.render("STOPPED", True, self.accent_color)
        
        self.screen.blit(state_text, (motor_rect.centerx - state_text.get_width()//2, 
                                     motor_rect.top + 40))
        
        # Draw servo control indicator (top center)
        servo_rect = pygame.Rect(self.width//2 - 100, 80, 200, 80)
        pygame.draw.rect(self.screen, self.card_color, servo_rect, 0, 10)
        
        # Draw servo status text
        servo_text = self.header_font.render("Steering Servo", True, self.primary_color)
        self.screen.blit(servo_text, (servo_rect.centerx - servo_text.get_width()//2, 
                                     servo_rect.top + 10))
        
        # Draw servo state
        if self.servo_state == "LEFT":
            servo_state_text = self.value_font.render("LEFT", True, self.success_color)
        elif self.servo_state == "RIGHT":
            servo_state_text = self.value_font.render("RIGHT", True, self.success_color)
        else:
            servo_state_text = self.value_font.render("CENTER", True, self.accent_color)
        
        self.screen.blit(servo_state_text, (servo_rect.centerx - servo_state_text.get_width()//2, 
                                           servo_rect.top + 40))
        ...
        # Draw control labels
        joystick_label = self.header_font.render("Steering Control", True, self.primary_color)
    ...
    def run(self):
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                # Joystick mouse events
                    ...
                    if self.dragging:
                        ...
                        # Determine servo state from position
                        ...
                        if distance < 20:  # Deadzone
                            ...
                            if -45 <= angle < 45:
                                ...
                                new_state = "DOWN"  # Not used for servo
                            elif -135 <= angle < -45:
                                new_state = "UP"  # Not used for servo
                            else:
                                new_state = "LEFT"
                        
                        # Only update if state changed
                        if new_state != self.servo_state:
                            self.set_servo_state(new_state)
            ...
            # Handle keyboard servo control
            if self.key_states[pygame.K_a] and not self.key_states[pygame.K_d]:
                new_servo_state = "LEFT"
            elif self.key_states[pygame.K_d] and not self.key_states[pygame.K_a]:
                new_servo_state = "RIGHT"
            else:
                new_servo_state = "CENTER"
            
            # Update servo state if changed
            if new_servo_state != self.servo_state:
                self.set_servo_state(new_servo_state)
            ...
            if self.key_states[pygame.K_w] and not self.key_states[pygame.K_s]:
                self.motor_state = "FORWARD"
            elif self.key_states[pygame.K_s] and not self.key_states[pygame.K_w]:
                self.motor_state = "BACKWARD"
            else:
                self.motor_state = "STOP"
                
            # Send motor command if state changed
            if self.motor_state != self.last_motor_state:
                self.send_command(f"MOTOR_{self.motor_state}")
